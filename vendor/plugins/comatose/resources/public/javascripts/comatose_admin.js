...
var ComatoseList = {
  ...
  item_hover : function(node, state, is_delete) {
    if( state == 'over') {
      $(node).addClass( (is_delete) ? 'hover-delete' : 'hover' );
    } else {
      $(node).removeClass( (is_delete) ? 'hover-delete' : 'hover' );
    }
  },
...
var ComatoseEditForm = {
  ...
  init : function(mode) {
    this.mode = mode;
    this.default_data = $(document.forms[0]).serialize();
    if(mode == 'new') {
  ...
  cancel : function(url) {
    var current_data = $(document.forms[0]).serialize();
